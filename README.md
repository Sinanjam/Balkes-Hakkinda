...
- Splash sırasında GitHub'daki son kararlı sürümü denetleyip yeni APK'ya yönlendirme.
...
- Sezon içinde resmi TFF puan tablosunu bütün haftalar arasında önceki/sonraki
  kontrolüyle gezme.
- Firebase, APK'ya gömülü keystore, yerel medya kopyası ve `google-services.json`
  olmadan küçük uygulama yapısı.
...
## Son kullanıcı sürümünü yayımla

```fish
fish release.fish
```

Bu tek komut kararlı APK'yı imzalar, imzayı doğrular ve GitHub Releases'a
**Balkes `<sürüm>.apk`** adıyla yükler. İlk çalıştırmada yayın anahtarı yalnızca
`~/.config/balkes/release/` altında oluşturulur; sonraki güncellemelerin
kurulabilmesi için bu klasörü güvenli bir yere yedekle.
